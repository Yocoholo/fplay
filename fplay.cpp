...
bool fplay::init_audio_decoder(AVFormatContext* fmt, int audio_idx, AudioContext& actx) {
    ...
    const AVChannelLayout *in_ch_layout = nullptr;
    AVChannelLayout tmp_in_ch_layout;
    if (actx.dec_ctx->ch_layout.nb_channels)
        in_ch_layout = &actx.dec_ctx->ch_layout;
    else {
        av_channel_layout_default(&tmp_in_ch_layout,
                                 actx.dec_ctx->ch_layout.nb_channels ? actx.dec_ctx->ch_layout.nb_channels : 2);
        in_ch_layout = &tmp_in_ch_layout;
    }
    
    ret = swr_alloc_set_opts2(&actx.swr_ctx,
                             &out_ch_layout, actx.state.dst_fmt, actx.state.dst_rate,
                             in_ch_layout, actx.dec_ctx->sample_fmt, actx.dec_ctx->sample_rate,
                             0, nullptr);
    if (ret < 0) ff_panic("swr_alloc_set_opts2", ret);
...
void fplay::cleanup_audio(AudioContext& actx) {
    SDL_CloseAudio();
    if (actx.swr_ctx) swr_free(&actx.swr_ctx);
    if (actx.state.swr) swr_free(&actx.state.swr);
